...
const GameDetailsScreen = () => {
  ...
  const handleShowAdviceSection = () => {
    if (token) setShowAdviceSection(true);
    else toggleModal();
  };
  ...
  const handleShareGame = () => {
    if (navigator.share) {
      ...
      navigator.clipboard
        ...
        .then(() => toast.success("Lien copié avec succès"))
  ...
  if (isMobile)
    return (
      <>
            ...
            <div>
              ...
              <div className="flex gap-1 mt-1">
                {game.game_platform.map((x) => (
                  <p key={x.platform.id} className="badge badge-outline badge-lg badge-accent bg-neutral uppercase">
                    {x.platform.name}
                  </p>
                ))}
              </div>
        ...
        {showAdviceSection && (
          <Modal handleClose={handleCloseAdviceSection}>
            <GameAdviceForm game={game} handleClose={handleCloseAdviceSection} advice={alreadyPostedAdvice} />
          </Modal>
        )}
      </>
    );
  ...
  return (
    <div className="flex w-4/5 mx-auto mt-2">
      ...
      <div className="flex-1 mt-2">
        {showAdviceSection ? <GameAdviceForm game={game} handleClose={handleCloseAdviceSection} advice={alreadyPostedAdvice} /> : RightSide}
      </div>
    </div>
  );
};
