...
export const AuthModal = (props: AuthModalProps) => {
  ...
  return (
    <div
      className="bg-base-100 h-screen fixed z-10 top-0 left-0 w-full p-4"
      onClick={(event) => event.stopPropagation()}
    >
      <h1 className="absolute capitalize text-lg">PixelWave</h1>
      <IoCloseSharp
        className="absolute z-50 right-0 text-4xl cursor-pointer"
        onClick={handleClose}
      />
      <div className="flex justify-center items-center">
        ...
        <img
          src="login.jpg"
          alt="auth"
          className="w-1/2 h-screen object-cover absolute right-0 top-0"
        />
      </div>
    </div>
  );
