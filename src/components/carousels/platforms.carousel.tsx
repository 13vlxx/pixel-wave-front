    ...
    return (
        <div className="font-semibold px-4 max-w-6xl mx-auto pb-8">
            ...
            <div className="grid grid-cols-3 gap-4 gap-x-6">
                {
                    platforms.map((x) => (
                        <div key={x.id} className="flex flex-col items-start justify-center carousel-item hover:scale-105 transition-all cursor-pointer">
                            <img className="size-full object-cover rounded-md" src={x.logo} alt={x.name} />
                        </div>
                    ))
                }
            </div>
